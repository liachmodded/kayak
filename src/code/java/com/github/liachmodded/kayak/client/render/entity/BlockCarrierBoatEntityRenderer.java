...
public final class BlockCarrierBoatEntityRenderer<T extends CarrierBoatEntity> extends AbstractBoatEntityRenderer<T> {
  ...
  protected void renderContent(T entity, float yaw, float tickDelta,
      MatrixStack matrices, VertexConsumerProvider vertexConsumerProvider, int ambientLightmapIndex) {
    ...
    if (state.getRenderType() != BlockRenderType.INVISIBLE) {
      ...
      MinecraftClient.getInstance().getBlockRenderManager()
          .renderBlockAsEntity(state, matrices, vertexConsumerProvider, lightmapIndex, OverlayTexture.DEFAULT_UV);
      matrices.pop();
    }
  }
}
